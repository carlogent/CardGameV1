...
public class DrawPhaseToMainPhaseEffect: CardEffectBase
{
    //create a struct that holds the entire data needed for a activation
    public struct DrawPhaseToMainPhaseEffectData
    {
    ...
    SortedDictionary<int, DrawPhaseToMainPhaseEffectData> activationsData; //the key (int) is the CardEffectActivation index in the CardEffectActivationChain //move to base.. or declare in interface

    public DrawPhaseToMainPhaseEffect(ICard? card = null): base(
    ...
    _isAnimated: false)
    {
        activationsData = new SortedDictionary<int, DrawPhaseToMainPhaseEffectData>(); //move to base
    ...
    {
        if(GameState.GetGamePhase() != GamePhase.DrawPhase || 
        ...
        activationsData.Keys.Last() > GameState.turnStartSequenceNumberList[^1]) // (once per turn) if the sequence number of the last effect activation is greater then the sequence number of when the turn changed, that means this effect activated this turn.
        {
            return false;
        }
    ...
    public override void Activate(int cardEffectActivationChainIndex){
        ...
        activationsData.Add(cardEffectActivationChainIndex, new DrawPhaseToMainPhaseEffectData(){
